...
var Sort = function (_React$Component) {
    ...
    function Sort(props) {
        ...
        _this.close = function (value, state) {

            // open status
            _this.context.updateSortOpen && _this.context.updateSortOpen(value, state);
    ...
    Sort.prototype.extendChildren = function extendChildren(child) {
        ...
        return _react2.default.cloneElement(child, {
            onChange: function onChange(result) {
                var ret = child.props.onChange && child.props.onChange(result);
                if (ret) {
                    _this2.setState({
                        label: ret.label
                    });
                    _this2.close(_this2.props.value, false);
                }
            }
        });
    };
...
Sort.PropTypes = {
    ...
    sortInfo: _propTypes2.default.array, // 0: 升序， 1： 降序
